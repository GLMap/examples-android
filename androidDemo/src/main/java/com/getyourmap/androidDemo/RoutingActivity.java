...
public class RoutingActivity extends Activity implements GLMapManager.StateListener {
    ...
    private void updateRoute() {
        RoutePoint pts[] = {
                new RoutePoint(departure, Float.NaN, true),
                new RoutePoint(destination, Float.NaN, true)};

        GLMapRouteData.ResultsCallback callback = new GLMapRouteData.ResultsCallback() {
            @Override
        ...
        switch (networkMode) {
            case Online:
                GLMapRouteData.requestOnlineRouteData(pts, routingMode, "en",
                        GLMapView.GLUnitSystem.International, callback);
                break;
            case Offline:
                GLMapRouteData.requestOfflineRouteData(getValhallaConfig(getResources()), pts,
                        routingMode, "en", GLMapView.GLUnitSystem.International, callback);
                break;
        }
    }
            ...
            public void onClick(View v) {
                if (mapToDownload != null) {
                    ...
                    if (task != null) {
                        ...
                        GLMapManager.DownloadDataSets(mapToDownload, GLMapInfo.DataSetMask.ALL, RoutingActivity.this);
                    }
    ...
    private void setMapView() {
        ...
        mapView.setCenterTileStateChangedCallback(new Runnable() {
            @Override
            public void run() {
                MapViewActivity.updateMapDownloadButton(mapView, btnDownloadMap, mapToDownload, localeSettings);
            }
        });
